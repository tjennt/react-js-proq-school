...
import {
  ...
  getDataSearch,
  filterData,
} from "../../../../../redux/actions/dataListPushFCM/index";
...
class DataListConfig extends Component {
      ...
      let columns = [
        ...
        },
        {
          ...
          cell: (row) => (
            <Chip
              className="m-0"
              color={!row.type ? "success" : "danger"}
              text={!row.type ? "Khuyến mãi" : "Khác"}
            />
          ),
        },
        {
          ...
          cell: (row) => (
            <Chip
              ...
              text={!row.status ? "Đã gửi" : "Chưa gửi "}
            />
          ),
        },
        {
        ...
        },
        {
          ...
          sortable: true,
          cell: (row) => (
            <p title={row.body} className="text-truncate text-bold-500 mb-0">
              {row.body}
            </p>
          ),
        },
  ...
  handleDelete = (row) => {
    ...
    if (data.length - 1 === 0) {
      history.push(
        `/pushFCM?page=${parseInt(parsedFilter.page - 1)}&perPage=${
  ...
  handlePagination = (page) => {
    ...
    history.push(`/pushFCM?page=${page.selected + 1}&perPage=${perPage}`);
