...
class AttendanceClassList extends React.Component {
  ...
  render() {
    ...
    for (const item of data) {
      let dataa = {};
      dataa.classCode = item.classCode;
    }
    let columnsTable = [];
        ...
        <CardHeader>
          ...
          {this.state.doingAtten ? (
            <Button
              color="primary"
              onClick={() =>
                this.props.history.push(localStorage.getItem("doingAtten"))
              }
            >
              Đang điểm danh
            </Button>
...
export default connect(mapStateToProps, { getDataClass })(AttendanceClassList);
